...
def train_svm(train_dataloader, test_dataloader):

    # 径向基函数 (RBF) 核函数
    svm_model = SVC(kernel='rbf')  # 初始化SVM模型
    ...
    for i in range(epoch):
        ...
        for batch_data in train_dataloader:
            ...
            for features, labels in zip(encoded_fusion, labels):
                predictions = svm_model.predict(features)  # 对单个样本进行预测
        ...
        if epoch_accuracy > best_train_accuracy:
            ...
            # 保存最优模型
            joblib.dump(svm_model, "./model_save/SVM_best_train_model.pkl")
            print("!!!!!!!!!!!!!!!!!!第 {} 轮的模型已保存!!!!!!!!!!!!!!!!!!!".format(i + 1))


        # test
        # 在测试阶段进行评估
        # svm_model.eval()  # 设置模型为评估模式
        ...
        for batch_data in test_dataloader:
            ...
            for feature, label, prediction in zip(features, labels, predictions):
                S1 = "H" if label == 0 else ("E" if label == 1 else "C")
                S2 = "H" if prediction == 0 else ("E" if prediction == 1 else "C")
                total_sov += calculate_sov(S1, S2)

                # 根据预测的标签将特征添加到相应类别的列表中
                if S1 == "H":
                    H_features.append(feature)
                elif S1 == "E":
        ...
        # 绘制散点图
        plt.scatter(H_features[:, 0], H_features[:, 1], c='r', marker='o', label='H')  # H类别的点，红色实心点
        plt.scatter(E_features[:, 0], E_features[:, 1], c='g', marker='o', label='E')  # E类别的点，绿色实心点
        plt.scatter(C_features[:, 0], C_features[:, 1], c='b', marker='o', label='C')  # C类别的点，蓝色实心点
        ...
        plt.title("SVM Classification on Test Data")
        # 保存图片到指定目录，存为 PNG 格式
        plt.savefig("picture/test_results.png", format='png')

        # 保存模型
        if test_accuracy > best_test_accuracy:
            best_test_accuracy = test_accuracy
            joblib.dump(svm_model, "./model_save/SVM_best_test_model.pkl")
            print("-----------------!!!!!!!!!!!!!!!!!!第 {} 轮的ACC最优!!!!!!!!!!!!!!!!!!!-----------------".format(i + 1))

if __name__ == '__main__':
    train_svm(train_dataloader, test_dataloader)
